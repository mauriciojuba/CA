...
public class ListPositionCtrl : MonoBehaviour
{
	...
	void Update()
	{
		...
		if ( !controlByButton && !controlByAxis)
		{
			if ( !isTouchingDevice )
				storeMousePosition();
			else
	...
	public void axisMove(){
		...
		if (inputAxis==1f && buttonCooldown<=0) {
			playerinfo.GetComponent<CustomizeText> ().id++;
			nextContent ();

			buttonCooldown = 0.2f;
		} else if (inputAxis==(-1f) && buttonCooldown<=0) {
			playerinfo.GetComponent<CustomizeText> ().id--;
			lastContent ();
			buttonCooldown = 0.2f;
		}
	...
	public void caseShift(){
		float inputAxis = Input.GetAxisRaw ("XBOX_buttonX");
	...
	{
		foreach (ListBox listbox in listBoxes) {
			int CID = listbox.getCurrentContentID();
			listbox.updateContent (ListBank.Instance.getListContent (CID));
